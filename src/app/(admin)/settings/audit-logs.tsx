...
export function AuditLogs() {
  ...
  return (
    <Card>
      ...
      <CardContent>
        {/* Custom tab buttons */}
        <div className="flex gap-1 mb-4 bg-muted rounded-md p-1 w-fit">
          <button
            onClick={() => { setType('action'); setAction('') }}
            className={`px-3 py-1.5 text-sm font-medium rounded-sm transition-colors ${
              type === 'action' ? 'bg-white text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
            }`}
          >
            📋 Действия
          </button>
          <button
            onClick={() => { setType('auth'); setAction('') }}
            className={`px-3 py-1.5 text-sm font-medium rounded-sm transition-colors ${
              type === 'auth' ? 'bg-white text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
            }`}
          >
            🔑 Вход/Изход
          </button>
        ...
        {loading ? (
          <div className="text-center py-8 text-muted-foreground text-xs">Зареждане...</div>
        ) : logs.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground text-xs">
            Няма записи. Логовете се създават при: зареждане, трансфер, заявки, настройки, потребители.
          </div>
        ) : type === 'action' ? (
